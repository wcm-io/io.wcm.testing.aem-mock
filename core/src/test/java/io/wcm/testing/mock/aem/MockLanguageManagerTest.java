...
public class MockLanguageManagerTest {
  ...
  public void getLanguageRootResources() {
    ...
    }, resources.stream().map(Resource::getPath).toArray());

    resources = languageManager.getLanguageRootResources(context.resourceResolver(), "/does/not/exist");
    assertNotNull(resources);
  ...
  public void getLanguageRootResources_respectContent() {
    ...
    assertArrayEquals(new String[] {
        ENGLISH_HOMEPAGE, FRENCH_HOMEPAGE, GERMAN_HOMEPAGE, EMEA_ENGLISH_HOMEPAGE
    }, resources.stream().map(Resource::getPath).toArray());

    resources = languageManager
      .getLanguageRootResources(context.resourceResolver(), EMEA_ENGLISH_HOMEPAGE + "/subpage/jcr:content", true);
    assertNotNull(resources);
    assertEquals(4, resources.size());
    assertArrayEquals(new String[] {
        EMEA_ENGLISH_HOMEPAGE, ENGLISH_HOMEPAGE, FRENCH_HOMEPAGE, GERMAN_HOMEPAGE
    }, resources.stream().map(Resource::getPath).toArray());
  ...
  public void getLanguageRoots() {
    ...
    assertArrayEquals(new String[] {
        ENGLISH_HOMEPAGE, FRENCH_HOMEPAGE
    }, roots.stream().map(Page::getPath).toArray());
  ...
  public void getCqLanguages() {
    ...
    assertArrayEquals(new Locale[] {
        Locale.ENGLISH, Locale.FRENCH
    }, languages.stream().map(Language::getLocale).toArray());
    ...
    assertArrayEquals(new Locale[] {
        Locale.ENGLISH, Locale.FRENCH
    }, languages2.stream().map(Language::getLocale).toArray());
  ...
  public void getAdjacentLanguageInfo() {
    ...
    assertArrayEquals(new Locale[] {
        Locale.ENGLISH, Locale.FRENCH
    }, adjacentLanguageInfo.keySet().stream().map(Language::getLocale).toArray());
